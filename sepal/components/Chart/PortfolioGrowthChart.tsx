...
import {Area, ComposedChart, Legend, Line, ResponsiveContainer, Tooltip} from "recharts";
...
export default function PortfolioGrowthChart(
  ...
  /**
   * Gets all of the chart keys
   */
  function getChartKeys() {
    if (data.length > 0) {
      return Object.keys(data[0]).filter(key => key !== 'date' && key !== 'portfolio');
    }
   ...
   * @param label label
   */
  const tooltip = ({ active, payload, label }: {active: boolean, payload: any, label: string}) => {
      ...
      <div className={'w-[100%]'}>
        <ResponsiveContainer width='100%' minHeight={300}>
          <ComposedChart data={chartData}>
            ...
            {
              hasMultipleAccounts() ?
                getChartKeys().map((item, itx) => {
                  return (
                    <Line key={item} type="monotone" dot={false} dataKey={item} strokeWidth={4} stroke={colors[itx]} />
                  )
                }) : null
            }
            <Area type="monotone" dot={false} dataKey="portfolio" stackId="1" stroke={`${Css.ColorPrimary}`} strokeWidth={4} fill="url(#color)"/>
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
