...
import {
  useTradeRecordControlsQuery,
  useTradeRecordsQuery,
} from "../query/queries";

/**
 * Computes the start and dates for lookup based on the filters
 */
function computeDates(selection: UserTradeRecordControlSelection): {
  start: string;
  end: string;
} {
  const getMoment = (val: "month" | "year") => {
    return moment(
      selection.year + "-" + selection.month + "-01",
      DateTime.ISODateLongMonthFormat,
    ).startOf(val);
  };
...
export function useTradeData(
  accNumber: number,
  filters: UserTradeRecordControlSelection,
  enabled: boolean,
) {
