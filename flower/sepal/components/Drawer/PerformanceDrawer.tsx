...
type Props = {
  ...
  onSubmit: () => void;
  onCancel: () => void;
  tradeRecordControls: { yearEntries: TradeRecordControlsYearEntry[] } | undefined;
};
...
export default function PerformanceDrawer({
  ...
  const matchingYear = tradeRecordControls?.yearEntries.find(ye => ye.year === userSelection.year) ?? null;
                ...
                <SelectContent>
                  {
                    matchingYear?.monthEntries.map((item, idx) => {
                      return (
                        <SelectItem key={item.uid + 'my' + (idx + 1)} value={item.month}
                ...
                <SelectContent>
                  {
                    tradeRecordControls?.yearEntries?.map((item, idx) => {
                      return (
                        <SelectItem key={item.uid + 'ye' + (idx + 1)} value={item.year}
