  ...
  return (
    <>
      {(!transactions || transactions.length === 0) &&
          <div className="text-center text-slate-500 mt-2 mb-6 text-sm">No recent transactions.</div>}
      {
                      ...
                      <TableRow key={item.uid} className={'hover:bg-transparent'}>
                        ...
                        <TableCell className={'text-right h-full'}>
                          <div className={'flex items-center justify-end'}>
                            {item.transactionStatus.label}&nbsp;<span
                            className={'inline-block ' + computeColors(item.transactionStatus.code)}><IconPointFilled
                            size={15}/></span>
                          </div>
                        </TableCell>
                      </TableRow>
