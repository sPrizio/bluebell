...
export default function AccountInsights(
   ...
   */
  const cell = (title: string, val: string, percent: number, hasToolTip: boolean) => {
    return (
      <div
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger className={'flex justify-end'}>
                      <span className={'inline-block'}><IconHelpSquareRounded className={'text-primary'} /></span>&nbsp;&nbsp;{title}
                    </TooltipTrigger>
                    <TooltipContent className={'bg-white border-1 border-gray-200 text-slate-700 shadow-sm'}>
                      <p>The drawdown is calculated using closed trades. Active trades are not considered <br/>during the calculations.</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )
  ...
  return (
    <div className={'grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-x-12'}>
      ...
      {cell('Biggest Loss', '$ ' + formatNumberForDisplay(insights.biggestLoss), insights.biggestLossDelta, false)}
      {cell('Largest Gain', '$ ' + formatNumberForDisplay(insights.largestGain), insights.largestGainDelta, false)}
      {cell('Current P & L', '$ ' + formatNumberForDisplay(insights.currentPL), insights.currentPLDelta, false)}
      {cell('Drawdown', '$ ' + formatNumberForDisplay(insights.drawdown), insights.drawdownDelta, true)}
      {cell('Max Profit', '$ ' + formatNumberForDisplay(insights.maxProfit), insights.maxProfitDelta, false)}
    </div>
  )
}