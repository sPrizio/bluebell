...
public class MarketNewsApiController extends AbstractApiController {
    ...
    @GetMapping("/get")
    public StandardJsonResponse<MarketNews> getNews(final @RequestParam("date") String date, final HttpServletRequest request) {
        validate(date);
    ...
    public StandardJsonResponse<List<MarketNews>> getNewsForInterval(final @RequestParam("start") String start, final @RequestParam("end") String end, final @RequestParam(required = false) String[] locales, final HttpServletRequest request) {
        validate(start, end);
