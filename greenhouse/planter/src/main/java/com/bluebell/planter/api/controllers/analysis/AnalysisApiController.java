...
public class AnalysisApiController extends AbstractApiController {
    ...
    public StandardJsonResponse<List<AnalysisResult>> getTimBucketsAnalysis(
        ...
        return account.map(value -> new StandardJsonResponse<>(true, this.analysisService.computeTimeBucketAnalysis(value, PlatformTimeInterval.THIRTY_MINUTE, AnalysisFilter.getAnalysisFilter(filter.toUpperCase()), isOpened), StringUtils.EMPTY)).orElseGet(() -> new StandardJsonResponse<>(true, null, CoreConstants.Validation.Account.ACCOUNT_NOT_FOUND));
    }
